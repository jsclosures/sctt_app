...
export default function HomePage() {
  ...
  // --- Selection tracking (uncontrolled DataGrid) ---
  ...
  const handleDelete = async () => {
    if (selectedRowIds.length === 0) {
      showMessage('No asset selected to delete', 'error');
      return;
    }
  ...
  const handleDuplicate = () => {
    if (selectedRowIds.length !== 1) {
      showMessage('Select exactly one asset to duplicate', 'error');
      return;
    }
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, height: '100%' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Select
            ...
            sx={{ minWidth: 150 }}
            onChange={(e) =>
              setCurrent((prev) => ({ ...prev, type: e.target.value }))
            }
          >
        ...
        {tabValue === 1 && (
          ...
          >
            ...
            <ResizableBox
              ...
              maxConstraints={[
                Infinity,
                fullScreen ? window.innerHeight - 200 : 800,
              ]}
              axis="y"
              resizeHandles={['s']}
            ...
            >
              <Editor
                ...
                onChange={(val) =>
                  setCurrent((prev) => ({ ...prev, script: val || '' }))
                }
                options={{
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          ...
          <Tooltip title={selectedRowIds.length > 1 ? `Delete ${selectedRowIds.length} selected assets` : 'Delete selected asset'}>
            <span>
              <Button
                variant="outlined"
                color="error"
          ...
          <Tooltip title={selectedRowIds.length === 1 ? 'Duplicate selected asset' : 'Select one asset to duplicate'}>
            <span>
              <Button
                variant="outlined"
                color="primary"
